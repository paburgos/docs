...
## <a href="Types-of-calibration.mdx">Types of Calibration</a> 
 ...
 To create a Equipment Template, select the main menu option with the same name:
• Now click “New” and enter the basic information, for this example, a  Brinell 
Hardness Testers POE will be search in the Equipment Type option: 
...
• In this example, we will create a Test Code for Brinell Hardness Testers POE. In the 
calibration type field, select the “Brinell Hardness Testers POE” option. Then, enter 
the  capacity ranges and the  unit of measure. Include  also a code,  description 
and if it is going to be used for accredited or non accredited calibrations. At the 
end click “Save”:
...
• Then click on the section “Brinell Calibration”:
 
• In this section you must fill in all the fields Ball Diam HBW, Load, Diam Avg, Tol +/-
(HBW) and Unc (HBW), which will be used to associate the standard to the wod 
generating calculations within each test point, each standard will generate a test 
point within the grid in the wod, for example:
...
calibration grids, which are composed of the Brinell As Found and Brinell As Left 
sections, must be configured: 

 
• In the Brinell As Found grid, two test points are generated, which are inherited from 
...
• In the Brinell As Left grid, two test points are generated, which are inherited from 
the previously selected standards, and are configured according to the data to be 
calibrated by the technician:
Also when saving this information, the Detail button can be clicked and the budget 
report will be generated for the uncertainty value of each test point: 
