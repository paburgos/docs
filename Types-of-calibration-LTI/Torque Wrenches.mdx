...
Wrenches PoE will be search in the Equipment Type option: 
...
will be activated and a resolution number must be added:• Now, in the “Other Information” section, a description can be included if needed, 
...
“New”: 
• In  this  example,  we  will  create  a  Test  Code  for  Torque Wrenches PoE.  In  the 
calibration type field, select the “Torque Wrenches PoE” option. Then, enter the 
capacity ranges and the unit of measure. Include also a code, description and if 
it is going to be used for accredited or non accredited calibrations. At the end 
click “Save”:4. To administrate Pieces Of Equipment (PoE), select the option with the same name in 
...
• Now click on the Configure Torque Transducer Standard - Accredited section, and 
add the range of values to configure the standard that will be associated to the wod, 
you must add mandatory values for the reference field, initial value, final value UoM, 
Class, for Note and Resolution are optional, for example: 
...
standards as optional, then save so that the system takes the changes made.: 
 
• Now in the Torque Wrenches Caliration section, we will configure the Torque 
Wrenches grid to enter the values in each corresponding test by the technician: 

• Open Torque Wrenches Grid and configurate the values for readings: 
